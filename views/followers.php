        ...
        <div class="p-6">
            <div class="mb-4 text-center">
                <div class="text-gray-800">
                    <span class="font-medium"><?php echo $user->username; ?></span>'s followers
                </div>
            </div>
                    ...
                    <?php foreach($followers as $follower): ?>
                        <div class="py-4 flex items-center justify-between">
                            <div class="flex items-center space-x-3">
                                <a href="/profile/<?php echo $follower['id']; ?>">
                                    <?php if($follower['profile_image']): ?>
                                        <img src="<?php echo $follower['profile_image']; ?>" alt="<?php echo $follower['username']; ?>" class="w-10 h-10 rounded-full object-cover">
                                    <?php else: ?>
                                ...
                                <div>
                                    <a href="/profile/<?php echo $follower['id']; ?>" class="font-medium text-gray-800 hover:text-primary transition duration-200"><?php echo $follower['username']; ?></a>
                                </div>
                            </div>
                            
                            <?php if($_SESSION['user_id'] != $follower['id']): ?>
                                <?php 
                                $current_user = new User($db);
                                $current_user->id = $_SESSION['user_id'];
                                $is_following = $current_user->isFollowing($follower['id']);
                                ?>
                                
                                <?php if($is_following): ?>
                                    <button class="unfollow-btn bg-gray-200 hover:bg-gray-300 text-gray-800 px-3 py-1 rounded-xl text-sm transition duration-200" data-user-id="<?php echo $follower['id']; ?>">Unfollow</button>
                                <?php else: ?>
                                    <button class="follow-btn bg-primary hover:bg-green-600 text-white px-3 py-1 rounded-xl text-sm transition duration-200" data-user-id="<?php echo $follower['id']; ?>">Follow</button>
                                <?php endif; ?>
                            <?php endif; ?>
                        </div>
                    <?php endforeach; ?>
